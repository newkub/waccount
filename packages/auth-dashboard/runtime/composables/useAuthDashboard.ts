...
/**
 * Main composable for Auth Dashboard
 * Provides authentication state and dashboard configuration
 */
export const useAuthDashboard = () => {
	const config = useRuntimeConfig().public.authDashboard

	// Auth state (will be populated by user's auth implementation)
	const user = useState<UserProfile | null>('auth-dashboard:user', () => null)
	const loading = useState<boolean>('auth-dashboard:loading', () => false)
	const isAuthenticated = computed(() => !!user.value)

	// Navigation state
	const currentTab = useState<string>('auth-dashboard:currentTab', () => 'account')
	const isMobileMenuOpen = useState<boolean>('auth-dashboard:mobileMenu', () => false)
