...
<template>
  <div>
    ...
    <div class="flex items-center gap-6">
      <div class="relative">
        <div v-if="props.user.avatar" class="w-20 h-20 rounded-full overflow-hidden bg-gray-100">
          <img :src="props.user.avatar" :alt="props.user.firstName || 'Avatar'" class="w-full h-full object-cover">
        </div>
        ...
        <div
          ...
          :class="[
            ...
            isDragOver ? 'border-primary-500 bg-primary-50/50' : 'border-transparent hover:border-gray-300 hover:bg-gray-500/20'
          ]"
        >
          <i class="i-mdi-camera text-white text-lg transition-opacity" :class="isDragOver ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'"></i>
        </div>
      </div>
