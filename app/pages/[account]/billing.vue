		...
		<div class="space-y-6">
			...
			<div class="bg-linear-to-r from-blue-50 to-purple-50 rounded-lg p-6 border border-blue-200">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					<div>
						...
						<ul class="space-y-1">
							<li v-for="feature in currentPlan?.features" :key="feature" class="flex items-center gap-2 text-sm text-gray-600">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
					<div v-for="(metric, key) in usage" :key="key" class="space-y-2">
						...
						<div v-if="metric.limit !== 'unlimited'" class="w-full bg-gray-200 rounded-full h-2">
							<div 
								:class="['h-2 rounded-full transition-all', getUsageColor(metric.percentage)]"
			...
			<div class="bg-gray-50 rounded-lg p-6">
				...
				<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
					...
					>
						...
						<p class="text-2xl font-bold text-gray-900 mt-1">{{ formatCurrency(plan.price) }}/month</p>
			...
			<div class="bg-blue-50 rounded-lg p-6 border border-blue-200">
				<div class="flex items-center gap-3 mb-4">
					...
					<h3 class="text-lg font-semibold text-gray-900">WorkOS Billing Features</h3>
				</div>
				
				<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
					<div class="flex items-start gap-3">
						<Icon name="mdi:receipt-text" class="w-5 h-5 text-green-600 mt-0.5" />
						<div>
							<p class="font-medium text-gray-900">Automated Invoicing</p>
							<p class="text-sm text-gray-600">Generate and send invoices automatically</p>
						</div>
					</div>
					<div class="flex items-start gap-3">
						...
						<div>
							...
							<p class="text-sm text-gray-600">Credit cards, ACH, wire transfers</p>
						</div>
					</div>
					<div class="flex items-start gap-3">
						<Icon name="mdi:file-chart" class="w-5 h-5 text-purple-600 mt-0.5" />
						<div>
							<p class="font-medium text-gray-900">Usage Analytics</p>
							<p class="text-sm text-gray-600">Track and analyze usage patterns</p>
						</div>
					</div>
				</div>
			</div>
