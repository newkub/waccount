...
<template>
	<div>
		...
		<div class="space-y-6">
			...
			<div class="bg-gray-50 rounded-lg p-6">
				...
				<div class="space-y-4">
					...
					<div class="flex items-center justify-between">
						...
						<div v-if="preferences.twoFactorEnabled" class="px-2 py-1 bg-green-100 text-green-700 text-xs rounded-full">
							Enabled
						</div>
						<button 
							v-else
			...
			<div v-if="organizations.length > 0" class="bg-linear-to-r from-blue-50 to-purple-50 rounded-lg p-6 border border-blue-200">
				<h3 class="text-lg font-semibold text-gray-900 mb-4">Organization Access</h3>
				<div class="space-y-3">
					<div v-for="org in organizations" :key="org.id" class="flex items-center justify-between">
						<div>
						...
						<NuxtLink :to="`/account/teams`" class="text-blue-600 hover:text-blue-700 text-sm font-medium">
							Manage →
						</NuxtLink>
					</div>
				</div>
